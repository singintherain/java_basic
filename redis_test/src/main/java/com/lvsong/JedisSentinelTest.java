...
public class JedisSentinelTest {
        ...
        public void sentinel() {
            ...
            try {
                ...
                Assert.assertEquals(master, masterFromSentinel);

                List<Map<String, String>> slaves = j.sentinelSlaves(MASTER_NAME);
                Assert.assertTrue(!slaves.isEmpty());
                Assert.assertEquals(master.getPort(), Integer.parseInt(slaves.get(0).get("master-port")));

                // DO NOT RE-RUN TEST TOO FAST, RESET TAKES SOME TIME TO... RESET
                Assert.assertEquals(Long.valueOf(1), j.sentinelReset(MASTER_NAME));
                Assert.assertEquals(Long.valueOf(0), j.sentinelReset("woof" + MASTER_NAME));
            } finally {
                j.close();
            }
        }
